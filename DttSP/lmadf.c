...
LMSR new_lmsr (CXB signal, int delay, REAL adaptation_rate, REAL leakage, int adaptive_filter_size, int filter_type)
{
	...
	lms->delay = delay;
	lms->delay_size = 512;
	lms->mask = lms->delay_size - 1;
	lms->delay_line = newvec_COMPLEX (lms->delay_size, "lmsr delay");
	lms->adaptation_rate = adaptation_rate;
	lms->leakage = leakage;
	lms->adaptive_filter_size = adaptive_filter_size;
	lms->adaptive_filter = newvec_COMPLEX (128, "lmsr filter"); // EW: why hard coded 128 here?
	lms->filter_type = filter_type;
	lms->delay_line_ptr = 0;
...
extern void lmsr_adapt(LMSR lms)
{
	...
	//fprintf(stderr, "afil[0]: %f\n", lms->adaptive_filter[0].re);
	//fflush(stderr);
	...
	for (i = 0; i < ssiz; i++)
	{
		...
		for (j = 0; j < asiz; j++)
		{
			...
			//if(j == 0)
			//	fprintf(stderr, "dptr:%u  k[%u:", dptr, k);
			sum_sq += Csqrmag (dlay (k));
			accum.re += lms->adaptive_filter[j].re * dlay (k).re;
			accum.im += lms->adaptive_filter[j].im * dlay (k).im;
		}
		...
		if(lms->filter_type == LMADF_NOISE)
			cssig(i) = accum;
		else // lms->filter_type == LMADF_INTERFERENCE
			cssig(i) = error;
		...
		{
			k = wrap (j);
			lms->adaptive_filter[j].re = lms->adaptive_filter[j].re * scl1 + error.re * dlay (k).re;
			lms->adaptive_filter[j].im = lms->adaptive_filter[j].im * scl1 + error.im * dlay (k).im;
		}
...
void dump_delay(LMSR lms)
{
	int i;
	FILE* file = fopen("lms_delay.csv", "w");
	for(i=0; i<lms->delay_size; i++)
	{
		fprintf(file, "%f, %f\n", lms->delay_line[i].re, lms->delay_line[i].im);
	}
