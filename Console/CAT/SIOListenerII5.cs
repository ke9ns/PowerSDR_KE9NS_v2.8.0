...
namespace PowerSDR
{
    public class SIOListenerII5
    {
        #region Constructor

        public SIOListenerII5(Console c)
        {
            console = c;
            ...
            SDRSerialPort5.serial_rx_event5 += new SerialRXEventHandler(SerialRXEventHandler5);

            if (console.CATEnabled5)  // if CAT is on, fire it up 
            {
                try
                {
                    enableCAT5();
        ...
        public void enableCAT5()
        {
            lock (this)
            {
                if (cat_enabled5) return; // nothing to do already enabled 
                cat_enabled5 = true;
            }
            Debug.WriteLine("==============CAT PORT OPEN");

            int port_num = console.CATPort5;

            SIO5 = new SDRSerialPort5(port_num);
            SIO5.setCommParms(console.CATBaudRate,
                            console.CATParity,
                            console.CATDataBits,
                            console.CATStopBits,
        ...
        public void disableCAT5()
        {
            lock (this)
            {
                if (!cat_enabled5) return; /* nothing to do already disabled */
                cat_enabled5 = false;
            }
        ...
        private bool cat_enabled5 = false;  // is cat currently enabled by user? 
        ...
        private void SerialRXEventHandler5(object source, SerialRXEvent e)
        {
