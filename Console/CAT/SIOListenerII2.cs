...
namespace PowerSDR
{
    public class SIOListenerII2
    {
        #region Constructor

        public SIOListenerII2(Console c)
        {
            console = c;
            ...
            SDRSerialPort2.serial_rx_event2 += new SerialRXEventHandler(SerialRXEventHandler2);

            if (console.CATEnabled2)  // if CAT is on, fire it up 
            {
                try
                {
                    enableCAT2();
        ...
        public void enableCAT2()
        {
            lock (this)
            {
                if (cat_enabled2) return; // nothing to do already enabled 
                cat_enabled2 = true;
            }
            Debug.WriteLine("==============CAT PORT OPEN");

            int port_num = console.CATPort2;

            SIO2 = new SDRSerialPort2(port_num);
            SIO2.setCommParms(console.CATBaudRate,
                            console.CATParity,
                            console.CATDataBits,
                            console.CATStopBits,
        ...
        public void disableCAT2()
        {
            lock (this)
            {
                if (!cat_enabled2) return; /* nothing to do already disabled */
                cat_enabled2 = false;
            }
        ...
        private void SerialRXEventHandler2(object source, SerialRXEvent e)
        {
